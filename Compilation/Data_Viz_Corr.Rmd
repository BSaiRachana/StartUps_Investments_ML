...
For the graph we can observe that many successful companies like Facebook, Twitter, Uber, YOuTube, WhatsApp intially started as startups after 2000. Also, the number of startup companies started in a year increased significantly from 2000.
...
For the graph we can observe that California (CA) state has the highest number of startups and the New York (NY) state is the second. Also, states like Massachusetts (MA), Texas (TX) and Washington (WA) have more number of startups.
...
Cities like San Francisco and New York have many number of startups.
...
We can observe that many startups established are still operating currently. Whereas, many are acquired and only few are closed which can be operating in future.
...
We can observe that successful companies are backed by Venture capital. Even top companies like facebook, youtube, google are backed by venture capital in the beginning.
...
We can observe that the statups started have good success rate till 2007 after which the success rate was reduced but after 2010 again it increased.
...
We can observe that every year few success companies are acquired, whereas as few are closed and the remaining stay operated.
